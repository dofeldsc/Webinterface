...
if (isset($search)) {
    ...
    if ($result_of_query->num_rows > 0) {
        ...
        $sql = "SELECT `id`,`name`,`members`,`level`,`bank`,`active` FROM `gang_system` WHERE `name` LIKE '%" . $search . "%' OR `id` = '" . $search . "' OR `members` LIKE '%" . $search . "%' ;";
        $result_of_query = $db_link->query($sql);
        $total_records = mysqli_num_rows($result_of_query);
        if ($pageNum > $total_records) $pageNum = $total_records;
        $sql = "SELECT `id`,`name`,`members`,`level`,`bank`,`active` FROM `gang_system` WHERE `name` LIKE '%" . $search . "%' OR `id` = '" . $search . "' OR `members` LIKE '%" . $search . "%' " . $max . " ;";
    } else {
        $sql = "SELECT `id` FROM `gang_system` WHERE `name` LIKE '%" . $search . "%'  OR `id` = '" . $search . "' OR `members` LIKE '%" . $search . "%' ;";
        $result_of_query = $db_link->query($sql);
        $total_records = mysqli_num_rows($result_of_query);
        if ($pageNum > $total_records) $pageNum = $total_records;
        $sql = "SELECT `id`,`name`,`members`,`level`,`bank`,`active` FROM `gang_system` WHERE `name` LIKE '%" . $search . "%' OR `id` = '" . $search . "' OR `members` LIKE '%" . $search . "%' " . $max . " ;";
    ...
    $sql = "SELECT `id` FROM `gang_system`;";
    $result_of_query = $db_link->query($sql);
    $total_records = mysqli_num_rows($result_of_query);
    if ($pageNum > $total_records) $pageNum = $total_records;
    $sql = "SELECT `id`,`name`,`members`,`level`,`bank`,`active` FROM `gang_system` " . $max . " ;";
}
$result_of_query = $db_link->query($sql);
if ($result_of_query->num_rows > 0) {
?>
...
<div class="content-panel">
    <table class="table table-striped table-advance table-hover">
        ...
        <tr>
            ...
            <th class="hidden-xs"><i class="fa fa-user"></i> Level</th>
        ...
        while ($row = mysqli_fetch_assoc($result_of_query)) {
            ...
            echo "<td>" . $row["name"] . "</td>";
            echo "<td class='hidden-xs'>$" . number_format($row["bank"],0, ",", ".") . "</td>";
